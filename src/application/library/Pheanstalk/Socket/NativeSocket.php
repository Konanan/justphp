...
class Pheanstalk_Socket_NativeSocket implements Pheanstalk_Socket
{
	...
	public function __construct($host, $port, $connectTimeout)
	{
		...
		if (!$this->_socket)
		{
            // LoggerManager::getSysLogger()->err(__FUNCTION__." beanstalkErr construct err ,errno=".var_export($errno,true).'&errstr='.var_export($errstr,true)." (connecting to $host:$port)");
            Logger::ins()->error(__FUNCTION__." beanstalkErr construct err ,errno=".var_export($errno,true).'&errstr='.var_export($errstr,true)." (connecting to $host:$port)");
	...
	public function read($length)
	{
		...
		while ($read < $length && !$this->_wrapper()->feof($this->_socket))
		{
			...
			if ($data === false)
			{
                ...
                // LoggerManager::getSysLogger()->error("fread Err: fread() returned false& this_socket=".var_export($this->_socket,true).'&length='.var_export($length,true).'&readlen='.var_export($read,true));
                Logger::ins()->error("fread Err: fread() returned false& this_socket=".var_export($this->_socket,true).'&length='.var_export($length,true).'&readlen='.var_export($read,true));
