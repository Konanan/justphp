...
class Pheanstalk_Socket_StreamFunctions
{
	...
	public function fread($handle, $length)
    {
        try{
            ...
            // LoggerManager::getSysLogger()->error("fread Err:".$e->getMessage()."& handle=".var_export($handle,true).'&length='.var_export($length,true));
            Logger::ins()->error("fread Err:".$e->getMessage()."& handle=".var_export($handle,true).'&length='.var_export($length,true));
